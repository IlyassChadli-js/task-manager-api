...
router.get('/users/me', auth, async (req, res) => { //middleware function as the second argument
    res.send(req.user)
})
...
router.delete('/users/me', auth, async (req, res) => {
    try {
        ...
        sendCancelationtEmail(req.user.email, req.user.name)
        res.send(req.user)
    } catch (e) {
        res.status(500).send()
    }
})
// add an img avatar to user
const upload = multer({
    limits: {
        fileSize: 1000000
    },
